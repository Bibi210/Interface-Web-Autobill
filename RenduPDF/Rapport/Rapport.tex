...
Dans ce projet, nous avons utilisé MiniML pour rédiger le lexer et le parser pour Autobill, ainsi que pour compléter la traduction de MiniML vers CBPV. Parallèlement, nous avons développé une interface web en utilisant React et NodeJS, en mode seul client et de client-server, qui présente de manière satisfaisante les résultats des étapes précédentes. Cette interface utilise également Minizinc pour analyser l'équation de l'analyse statique, afin d'obtenir les bornes mémoire du programme. Nous avons fait la comparaision sur les resultats obtenu par côte client et côte server, et nous avons eu le même resultat.\\
      
      \subsection{Lien avec ce projet et cours}\label{Lien avec ce projet et cours}}
...
Nous avons également rencontré quelques difficultés pour faire avancer le projet. Tout d'abord, nous n'avons pas été en mesure de construire un pipeline MiniML $\rightarrow$ LCBPV $\rightarrow$ Autobill $\rightarrow$ Équation $\rightarrow$ Résolution, avec le lien Autobill $\rightarrow$ Équation est toujours en progress. Et finalement nous ne pouvons pas comparer notre solution avec celle de Jan Hoffmann et l'interface de RAML. Deuxièmement, notre projet avance en parallèle avec Hector et nous devons être totalement immergés dans le sujet de recherche. Mais nous devions aussi travailler sur plusieurs projets en même temps, et nous avons eu beaucoup de challenge de gestion de temps. Et puis, il est notre premier contact avec les références et la rédaction d'une bibliographie, et nous ne savions pas comment les citer correctement au début, mais notre tuteur nous a donné des instructions détaillées sur la manière de les citer correctement.\\

\subsection{Ce qu'on a appris}\label{Ce qu'on a appris}}
Cependant, ce projet nous a beaucoup apporté. Tout d'abord, notre capacité à écrire du code a été améliorée et nous avons acquis une meilleure compréhension des principes de compilation et de la composition des langages de programmation. Deuxièmement, nous avons lu un certain nombre d'articles dans lesquels nous avons appris la grande contribution de Jan Hofmann à l'étude de l'analyse automatique des ressources amorties et les principes de call-by-push-value. Notre capacité à gérer le temps et à travailler en mode multiple-threads, comme mentionné ci-dessus, a également été améliorée, de même que notre capacité d'introduire des références de manière disciplinement.\\
